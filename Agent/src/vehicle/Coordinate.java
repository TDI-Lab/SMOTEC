	...
	public class Coordinate { // extends Map {
		...
		public boolean isWithinLimitPosition(Coordinate c) {
			if (c.getCoordX() >= Constants.MAX_X ||
				c.getCoordX() < Constants.MIN_X ||
				c.getCoordY() >= Constants.MAX_Y ||
				c.getCoordY() < Constants.MIN_Y)
				return false;
			else
				return true;
		}

		public static void disableSmartThing(Vehicle smartThing) {
			smartThing.setCoord(-1, -1);
			smartThing.setStatus(false);
		...
		public static void newCoordinate(Vehicle smartThing) {
			...
			if (smartThing.getTravelTimeId() < path.size()) {
				...
				if (x < Constants.MIN_X || y < Constants.MIN_Y || x >= Constants.MAX_X || y >= Constants.MAX_Y) {//zeinab
					...
					System.out.println("Agent "+smartThing.getMyId()+" out of area.");
				}
				else {
					...
					System.out.println("new coordinate : x="+ x+" "+"y="+ y+" time="+smartThing.getTravelTimeId()+" speed="+speed+" direction="+direction);
				}
			}
			else {
				System.out.println("Agent "+smartThing.getMyId()+" end of mobility path.");
				disableSmartThing(smartThing);
			}
		}
	
		public static void setInitialCoordinate(Vehicle smartThing) {
			...
			if (!path.isEmpty()) {
				...
				if (x < Constants.MIN_X || y < Constants.MIN_Y || x >= Constants.MAX_X || y >= Constants.MAX_Y) {
					...
					smartThing.getCoord().setCoordY(y);
					smartThing.setSpeed(speed);
					System.out.println("User coordinate initializing: x="+ x+" "+"y="+ y+" time="+time+" speed="+speed+" direction="+direction);
		...
		public static void newCoordinate(Vehicle smartThing, int add, Coordinate coordDevices) {
